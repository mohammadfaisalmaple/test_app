...
class ExtSmsActivateProvider
{
    ...
    public function __construct($saby, $country, $emulator_port)
    {
        ...
        $this->setControlCenterApi();
        //$this->api_key = array_rand($this->apiKeys, 1);
        $this->api_key = $this->_getSmsActivateCode();
        $this->service = 'wa'; // BIP
    ...
    public function setControlCenterApi($controlCenterApi = null)
    {
        myLog('RUNNING >>> ' . __FUNCTION__ . " >>>>  IN " . __CLASS__);
        $this->controlCenterApi = "http://65.108.105.115/MapleVoipSocket/apiManager";
    }
    ...
    public function _getSmsActivateCode()
    {
        myLog('RUNNING >>> ' . __FUNCTION__ . " >>>>  IN " . __CLASS__);
        $ch = curl_init();
        $url = $this->controlCenterApi . "/getSmsActivateCode";
        myLog("THE URL REQUESTED --> " . __FUNCTION__ . " IN  " . __CLASS__ . " -> " . $url);
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $output = curl_exec($ch);
        curl_close($ch);
        $outputArray = json_decode($output);
        return $outputArray->api_key;
    }
}
