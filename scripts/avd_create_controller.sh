...
function emulatorWipeStart() {
    nohup ~/Android/Sdk/emulator/emulator @${AVD_NAME} -port $CONSOLE_PORT -no-snapshot -camera-back none -camera-front none -memory 2048 -cache-size 1000 -partition-size 8192 -shell -qemu -allow-host-audio >~/avd.log 2>&1 &
}
emulatorWipeStart
sleep 10
